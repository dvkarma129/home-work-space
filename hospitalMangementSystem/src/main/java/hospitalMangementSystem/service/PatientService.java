...
public class PatientService {
	...
	public Patient updatePatient(Long id, Long deptId, Patient model) throws Exception {
		Patient patient = patientRepo.findById(id).get();
		Optional<Department> department = departmentRepo.findById(deptId);
		if (department.isEmpty()) {
			throw new Exception("Department not found");
		}
		patient.setDepartment(department.get());
		patient.setStatus(model.getStatus());
		patientRepo.save(patient);
		
		History newHistory = historyRepo.findTopByPatientOrderByDateAdmittedDesc(patient);
		System.out.println(newHistory);
		newHistory.setDateDischarged(LocalDateTime.now());
		newHistory.setDepartment(departmentRepo.findById(deptId).get());
		historyRepo.save(newHistory);
